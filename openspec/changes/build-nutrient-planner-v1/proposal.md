...
- **Phase calculation**: cycle day from `--last-period`/`--cycle-length`; phase boundaries are Menstrual (fixed days 1–5), Ovulatory (a 3-day window centered on `cycle_length − 14`), with Follicular and Luteal filling the remaining days. A 21-day cycle is valid and has zero Follicular days by design — handled, not rejected.
...
- **`.gitignore`** covers `history.json` and `history.json.bak` (period dates, public repo).
...
- New files: `plan.py`, `nutrition_rules.json` (stub data), `.gitignore` entries for `history.json`/`history.json.bak` (already added).
